...
const getTodaysConversionRate = async exchangeApiKey => {
  console.log("gathering exchange rates...");
  // https://exchangerate.host
  const todaysRates = await axios.get(
    "http://api.exchangerate.host/live?source=USD&currencies=AUD&access_key=" +
      exchangeApiKey
  );
  console.log("todays rate", todaysRates.data.quotes.USDAUD);
  return todaysRates.data.quotes.USDAUD;
};

const tryGetTodaysConversionRate = async exchangeApiKey => {
  try {
    return await getTodaysConversionRate(exchangeApiKey);
  } catch (e) {
    console.warn(e);
    return null;
  }
